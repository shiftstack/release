...
scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i /tmp/id_rsa-proxy ${PROXY_USER}@${LEASED_RESOURCE}:~/kubeconfig /tmp/secret/rhoso_kubeconfig
...
- name: "Run shiftstack"
  ...
  vars:
    ...
    # REMOVE ONCE MERGED:
    cifmw_shiftstack_qa_gerrithub_change: "refs/changes/71/1208771/3"
  tasks:
    - block:
