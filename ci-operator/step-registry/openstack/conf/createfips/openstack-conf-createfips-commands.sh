...
API_FIP_AND_ID=$(openstack floating ip create --description $CLUSTER_NAME.api-fip $OPENSTACK_EXTERNAL_NETWORK --format value -c 'floating_ip_address' -c 'id')
echo ${API_FIP_AND_ID} |awk '{print $1}' > ${SHARED_DIR}/API_FIP_IP
echo ${API_FIP_AND_ID} |awk '{print $2}' > ${SHARED_DIR}/API_FIP_UID
cp ${SHARED_DIR}/API_FIP_IP ${ARTIFACT_DIR}
cp ${SHARED_DIR}/API_FIP_UID ${ARTIFACT_DIR}
#Mark the fip for deletion
echo ${API_FIP_AND_ID} |awk '{print $2}' >> ${SHARED_DIR}/DELETE_FIPS
